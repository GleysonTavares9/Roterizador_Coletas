...
export default function DriverRouteExecution() {
    ...
    const fetchRouteData = async () => {
        ...
        try {
            ...
            // Fetch points ordered by sequence
    ...
    const submitKm = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('routes').update(update).eq('id', routeId);
    ...
    const handleArrivedAtPoint = async (point: any) => {
        ...
        try {
            const { error } = await supabase
                .from('route_points')
                .update({ status: 'arrived', visited_at: new Date().toISOString() })
                .eq('id', point.id);
            if (error) throw error;
            fetchRouteData();
    ...
    const submitCollection = async () => {
        ...
        setActionLoading(true);
        try {
            const { error } = await supabase.from('route_points').update({
                status: 'collected',
    ...
    return (
        <div className="min-h-screen bg-slate-100 flex flex-col">
            ...
            <div className="bg-[#0c3773] p-6 text-white sticky top-0 z-50 shadow-lg relative overflow-hidden">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <Truck className="w-7 h-7 text-blue-100" />
                                <h1 className="text-3xl font-extrabold tracking-tight leading-none">{route?.vehicle_plate}</h1>
                            </div>
                            <div className="text-blue-100 text-sm font-medium pl-10">
                ...
                <div className="bg-white/10 backdrop-blur-md rounded-lg border border-white/10 p-4 relative z-10">
                    <div className="grid grid-cols-4 gap-0 divide-x divide-white/20">
                        {/* Pontos */}
                        <div className="flex flex-col items-center justify-center px-1">
                            <div className="flex items-center gap-1.5 text-[10px] uppercase text-blue-100 font-bold mb-1 tracking-wider opacity-80">
                                <MapPin className="w-3 h-3" /> Pts
                            </div>
                            <span className="font-bold text-xl leading-none">
                                {points.filter(p => p.status === 'collected').length}
                                <span className="text-sm font-normal text-blue-100/60">/{points.length}</span>
                            </span>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center px-1">
                            ...
                            <div className="font-bold text-xl leading-none">
                                {displayDistance.toFixed(1)} <span className="text-sm font-medium text-blue-100/80">km</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center px-1">
                            ...
                            <div className="font-bold text-xl leading-none whitespace-nowrap">
                                {hours}h<span className="text-base">{minutes > 0 ? minutes : ''}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center px-1">
                            ...
                            <div className="font-bold text-xl leading-none">
                                {totalWeight.toFixed(0)} <span className="text-sm font-medium text-blue-100/80">kg</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {isRouteStarted && (
                <div className="relative h-1.5 bg-slate-200 w-full">
                    <div
            ...
            <div className="flex-1 p-4 max-w-md mx-auto w-full space-y-6 pb-32 mt-4">
                {/* Route Actions */}
                {!isRouteStarted && (
                                                ...
                                                {point.status === 'en_route' && (
                                                    <div className="w-full space-y-3">
                                                        <div className="grid grid-cols-2 gap-3">
                                                            <Button
                                                                variant="outline"
                                                                className="h-14 border-2 border-slate-200 bg-white hover:bg-slate-50 text-slate-800 font-bold shadow-sm hover:shadow-md transition-all"
                                                                onClick={() => {
                                                            ...
                                                            <Button
                                                                variant="outline"
                                                                className="h-14 border-2 border-cyan-300 bg-gradient-to-br from-cyan-50 to-cyan-100 hover:from-cyan-100 hover:to-cyan-200 text-slate-800 font-bold shadow-sm hover:shadow-md transition-all"
                                                                onClick={() => {
                                                                    const query = encodeURIComponent(point.address || '');
                                                        ...
                                                        <Button
                                                            className="w-full bg-yellow-600 hover:bg-yellow-700 h-14 text-lg shadow-md transition-transform active:scale-95 animate-pulse"
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-[#162562] border-t border-blue-900/30 px-4 py-2 flex justify-around items-center z-50 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.3)] pb-4 md:pb-2">
                <Button variant="ghost" className="flex flex-col items-center h-auto py-2 gap-0.5 w-24 hover:bg-white/10 rounded-xl" onClick={() => navigate(-1)}>
                    <Home className="w-7 h-7 text-white" />
                    <span className="text-xs uppercase font-bold text-white">Voltar</span>
                </Button>

                <div className="w-px h-8 bg-white/20"></div>

                <Button variant="ghost" className="flex flex-col items-center h-auto py-2 gap-0.5 w-24 hover:bg-white/10 rounded-xl" onClick={() => setChatOpen(true)}>
                    <div className="relative">
                        <MessageSquare className="w-7 h-7 text-white" />
                    </div>
                    <span className="text-xs uppercase font-bold text-white">Chat</span>
                </Button>

                <div className="w-px h-8 bg-white/20"></div>

                <Button variant="ghost" className="flex flex-col items-center h-auto py-2 gap-0.5 w-24 hover:bg-white/10 rounded-xl" onClick={startCall}>
                    <Phone className="w-7 h-7 text-white" />
                    <span className="text-xs uppercase font-bold text-white">Ligar</span>
                </Button>

                <div className="w-px h-8 bg-white/20"></div>

                <Button variant="ghost" className="flex flex-col items-center h-auto py-2 gap-0.5 w-24 hover:bg-white/10 rounded-xl" onClick={() => window.location.reload()}>
                    <RefreshCw className="w-7 h-7 text-white" />
                    <span className="text-xs uppercase font-bold text-white">Atualizar</span>
                </Button>
            </div>
            {/* --- Voice Call Overlay --- */}
            ...
            {callStatus !== 'idle' && (
                <div className="fixed inset-0 z-[60] bg-slate-900/95 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
...
function DriverChat({ routeId, open, onClose }: { routeId: string, open: boolean, onClose: () => void }) {
    ...
    useEffect(() => {
        if (!open) return;
        supabase.from('route_messages').select('*').eq('route_id', routeId).order('created_at', { ascending: true })
            .then(({ data }) => {
                if (data) setMessages(data);
                // Removido update de read_by_driver pois coluna não existe
            });
    ...
    const send = async () => {
        ...
        const { error } = await supabase.from('route_messages').insert({
            ...
            message: msg,
            sender_type: 'driver'
            // Removidos campos de leitura que não existem no banco
        });
    ...
    return (
        <div className="fixed inset-0 z-50 bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
            ...
            <div className="p-3 bg-white border-t flex gap-2 fixed bottom-0 left-0 right-0 z-50 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.1)]">
